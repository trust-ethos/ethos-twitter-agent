...
// Polling endpoints removed - service now only supports cron-triggered single requests
...
router.post("/cron/poll-mentions", async (ctx) => {
  try {
    ...
    ctx.response.body = {
      status: "success",
      message: "Mention check completed",
      timestamp: new Date().toISOString()
    };
  } catch (error) {
    console.error("❌ HTTP Cron mention check failed:", error);
    ctx.response.status = 500;
    ctx.response.body = {
      status: "error",
      message: "Cron mention check failed",
      error: error.message,
      timestamp: new Date().toISOString()
    };
  }
...
if (usePolling) {
  console.log("🔄 Running in CRON mode (good for Basic Twitter API plan)");
  console.log("🕐 Checking mentions every 3 minutes via Deno Deploy Cron");
  console.log(`🔗 Webhook URL: http://localhost:${port}/webhook/twitter (disabled in cron mode)`);
