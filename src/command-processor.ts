...
import { AskRateLimiter } from "./ask-rate-limiter.ts";
...
export class CommandProcessor {
  ...
  private askRateLimiter: AskRateLimiter;
  ...
  constructor(twitterService: TwitterService) {
    ...
    const cliService = new EthosCliService();
    this.askService = new AskService(cliService);
    this.askRateLimiter = new AskRateLimiter();
  }
  ...
  /** Public accessor for ask rate limiter (used by cron cleanup) */
  get askLimiter(): AskRateLimiter {
    return this.askRateLimiter;
  }
   ...
   */
  private getTwitterAvatarUrl(
    userId: string,
    size: "_normal" | "_bigger" | "_mini" | "_400x400" = "_bigger",
  ): string {
    ...
    return `https://abs.twimg.com/sticky/default_profile_images/default_profile_${
      size === "_400x400"
        ? "400x400"
        : size === "_bigger"
        ? "bigger"
  ...
  async parseCommand(
    ...
    const validCommands = ["profile", "save", "help", "grifter?", "ask"];
    ...
    if (!ethosAgentMentioned) {
      console.log(
        `ℹ️ @ethosAgent not in initial mention group: [${
    ...
    if (validCommands.includes(potentialCommand)) {
      ...
      if (resolvedCommand) {
        console.log(
          `✅ AI resolved command: "${commandText}" → "${resolvedCommand}"`,
        );
      } else {
        console.log(
          `ℹ️ Could not resolve intent for: "${commandText}" in "${originalText}"`,
        );
        return null;
      }
    }
    ...
    const secondToLastMention = initialMentions.length >= 2
      ? initialMentions[initialMentions.length - 2]
      : null;
    ...
    if (isSaveTargetCommand) {
      ...
      if (!ethosAgentInCorrectPosition) {
        console.log(
          `ℹ️ For save target command, @ethosAgent must be last or second-to-last mention in group: [${
            initialMentions.join(", ")
          }]`,
        );
        return null;
      ...
      console.log(
        `✅ Save target command: @ethosAgent found in valid position (last: ${lastMention}, second-to-last: ${secondToLastMention})`,
      );
    } else {
      // For all other commands, @ethosAgent must be the LAST mention
      if (!lastMention.includes("@ethosagent")) {
        console.log(
          `ℹ️ @ethosAgent not the last mention in group: [${
            initialMentions.join(", ")
          }]. Last: ${lastMention}`,
        );
        return null;
    ...
    console.log(
      `🎯 Found valid command: ${resolvedCommand} (after mentions: [${
        initialMentions.join(", ")
      }])`,
    );
    return {
  ...
  async processCommand(
    command: Command,
    allUsers?: TwitterUser[],
  ): Promise<CommandResult> {
    console.log(
      `🎯 Processing command: ${command.type} with args:`,
      command.args,
    );

    // Check if the user is blocked
    const isBlocked = await this.blocklistService.isBlocked(
      command.mentionedUser.username,
      command.mentionedUser.id,
    );
    if (isBlocked) {
      console.log(
        `🚫 Ignoring command from blocked user: @${command.mentionedUser.username} (${command.mentionedUser.id})`,
      );
      return {
        success: false,
    ...
    try {
      switch (command.type) {
        ...
        case "ask":
          return await this.handleAskCommand(command, allUsers);
        ...
        default:
          return {
            ...
            replyText:
              `I don't recognize the command "${command.type}". Try @ethosAgent help for available commands.`,
          };
      }
    } catch (error) {
      console.error(
        `❌ Unexpected error processing ${command.type} command:`,
        error,
      );
      ...
      const knownCommands = ["profile", "help", "save", "grifter?", "ask"];
  ...
  private async handleProfileCommand(
    ...
    try {
      ...
      if (isReply && tweet.in_reply_to_user_id) {
        ...
        const originalAuthor = allUsers?.find((user) =>
          user.id === tweet.in_reply_to_user_id
        );
        ...
        if (originalAuthor) {
          ...
          analysisContext =
            `analyzing the profile of ${targetName} (@${targetUsername}) as requested by @${mentionerUsername}`;
        } else {
          ...
          return {
            ...
            replyText:
              `I couldn't find information about the original tweet author. Please make sure you're replying to a valid tweet.`,
          };
        }
      } else {
        ...
        analysisContext =
          `analyzing the profile of ${targetName} (@${targetUsername}) at their request`;
      }

      console.log(`👤 Processing profile command: ${analysisContext}`);

      // Fetch Ethos stats for the target user
      const ethosResponse = await this.ethosService.getUserStats(
        targetUsername,
      );
      ...
      if (ethosResponse.success && ethosResponse.data) {
        ...
        replyText = this.ethosService.formatStats(
          ethosResponse.data,
          targetName,
          targetUsername,
        );
        ...
        replyText = this.ethosService.getFallbackMessage(
          targetName,
          targetUsername,
          ethosResponse.error,
        );
        console.log(
          `ℹ️ No Ethos data for ${targetUsername}: ${ethosResponse.error}`,
        );
      }
   ...
   */
  private async handleGrifterCommand(
    command: Command,
    allUsers?: TwitterUser[],
  ): Promise<CommandResult> {
    try {
      ...
      if (isReply && tweet.in_reply_to_user_id) {
        ...
        if (originalAuthor) {
          ...
          analysisContext =
            `analyzing grifter status of ${targetName} (@${targetUsername}) as requested by @${mentionerUsername}`;
        } else {
          // Fallback if we can't find the original author in webhook data
          return {
            success: false,
        ...
        analysisContext =
          `analyzing grifter status of ${targetName} (@${targetUsername}) at their request`;
      }
      ...
      if (ethosResponse.success && ethosResponse.data) {
        ...
        replyText = this.ethosService.formatGrifterAssessment(
          ethosResponse.data,
          targetName,
          targetUsername,
        );
        ...
        const topReviewResponse = await this.ethosService.getTopReview(
          targetUserId,
        );
        if (
          topReviewResponse.success && topReviewResponse.review &&
          topReviewResponse.review.comment
        ) {
        ...
        replyText = this.ethosService.getGrifterFallbackMessage(
          targetName,
          targetUsername,
          ethosResponse.error,
        );
        console.log(
          `ℹ️ No Ethos data for grifter check ${targetUsername}: ${ethosResponse.error}`,
        );
      }
  ...
  private async handleHelpCommand(command: Command): Promise<CommandResult> {
    try {
      ...
      if (command.args.length > 0) {
        console.log(
          `ℹ️ Help command ignored - has additional arguments: ${
            command.args.join(" ")
          }`,
        );
        return {
      ...
      console.log(
        `📚 Processing help command for @${command.mentionedUser.username}`,
      );
...
**ask anything** - Ask any question about Ethos reputation data
   • "who vouched for @user?"
   • "what reviews does @user have?"
   • "show me the trust market for @user"
...
Learn more about Ethos at https://ethos.network`;
  ...
  private async handleSaveCommand(
    ...
    try {
      ...
      const isRateLimited = await this._storageService.isRateLimited(
        mentionerUserId,
        "save",
      );
      if (isRateLimited) {
        console.log(
          `🚨 Rate limit hit: @${mentionerUsername} (${mentionerUserId}) exceeded save command limit`,
        );
        return {
          success: false,
          message: "Save command rate limit exceeded",
          replyText:
            `You've reached the limit of 5 save commands per hour. Please try again later.`,
        };
      }
      ...
      console.log(
        `   allUsers provided: ${allUsers ? allUsers.length : 0} users`,
      );
      ...
      if (!tweet.referenced_tweets || tweet.referenced_tweets.length === 0) {
        return {
          ...
          replyText:
            `To save a tweet as a review, you need to reply to the tweet you want to save. Try replying to someone's tweet with "@ethosAgent save".`,
        };
      }
      ...
      const sentimentIndex = remainingArgs.findIndex((arg) =>
        validSentiments.includes(arg.toLowerCase())
      );

      if (sentimentIndex !== -1) {
        reviewScore = remainingArgs[sentimentIndex].toLowerCase() as
          | "positive"
          | "negative"
          | "neutral";
      ...
      let targetUserId: string;
      if (
        remainingArgs.length >= 2 && remainingArgs[0].toLowerCase() === "target"
      ) {
        ...
        if (!mentionMatches || mentionMatches.length === 0) {
          return {
            ...
            replyText:
              `No @mention found for target user. Use "@ethosAgent save target @username" format.`,
          };
        }
        ...
        if (!usernameMatch) {
          return {
            ...
            replyText:
              `Invalid username format. Use "@ethosAgent save target @username" format.`,
          };
        }
        ...
        saveContext =
          `saving tweet as review to ${targetName}'s profile as requested by @${mentionerUsername}`;

        // For target case, we need to look up the user ID from the username
        // First try to find them in allUsers (from webhook)
        let targetUser: TwitterUser | undefined = allUsers?.find((user) =>
        ...
        if (!targetUser) {
          ...
          console.log(
            `🔍 @${targetUsername} not in webhook data, looking up via Twitter API...`,
          );
          try {
            const apiUser = await this.twitterService.getUserByUsername(
              targetUsername,
            );
            if (!apiUser) {
              return {
                ...
                replyText:
                  `I couldn't find the user @${targetUsername}. Please make sure the username is correct.`,
              };
            }
            targetUser = apiUser;
            console.log(
              `✅ Found @${targetUsername} via API: ${targetUser.name} (ID: ${targetUser.id})`,
            );
          } catch (error) {
            console.error(
              `❌ Failed to lookup @${targetUsername} via API:`,
              error,
            );
            return {
              success: false,
              ...
              replyText:
                `I couldn't find information about @${targetUsername}. Please make sure the username is correct.`,
            };
          }
        } else {
          console.log(
            `✅ Found @${targetUsername} in webhook data: ${targetUser.name} (ID: ${targetUser.id})`,
          );
        }
        ...
        targetUserId = originalAuthor.id;
        saveContext =
          `saving tweet as review to ${targetName} (@${targetUsername})'s profile as requested by @${mentionerUsername}`;

        // Log ignored extra text for debugging
        if (remainingArgs.length > 0) {
          console.log(
            `ℹ️ Ignoring extra text in save command: "${
              remainingArgs.join(" ")
            }"`,
          );
        }
      ...
      if (targetUsername.toLowerCase() === "ethosagent") {
        console.log(
          `🚫 Agent self-review blocked: @${mentionerUsername} trying to save review to @ethosAgent`,
        );
        return {
          success: false,
          message: "Cannot review the agent itself",
          replyText:
            `I can't save a review to my own profile! Try a different target.`,
        };
      }

      // 🚨 ANTI-ABUSE: Check if user is trying to review themselves with positive sentiment
      if (
        targetUsername.toLowerCase() === mentionerUsername.toLowerCase() &&
        reviewScore === "positive"
      ) {
        console.log(
          `🚨 Self-review abuse detected: @${mentionerUsername} trying to review themselves positively`,
        );
        ...
        console.log(
          `🚨 Converted positive self-review to negative sentiment for @${mentionerUsername}`,
        );
      } else if (
        targetUsername.toLowerCase() === mentionerUsername.toLowerCase()
      ) {
        console.log(
          `ℹ️ Self-review detected but allowing ${reviewScore} sentiment for @${mentionerUsername}`,
        );
      }

      // Check if the reviewer (person making the save request) has a valid Ethos profile
      console.log(
        `🔍 Validating Ethos profile for reviewer: ${mentionerUsername} (ID: ${command.mentionedUser.id})`,
      );
      const profileCheck = await this.ethosService.checkUserProfile(
        command.mentionedUser.id,
      );
      ...
      if (!profileCheck.hasProfile) {
        console.log(
          `❌ User ${mentionerUsername} does not have a valid Ethos profile`,
        );
        return {
          success: false,
          message: "Reviewer does not have an Ethos profile",
          replyText:
            "I'm sorry, only users with an Ethos profile can use this functionality",
        };
      }

      console.log(
        `✅ User ${mentionerUsername} has a valid Ethos profile (profileId: ${profileCheck.profileId})`,
      );

      // Find the original tweet that's being replied to
      let originalTweetId: string;
      const repliedTweet = tweet.referenced_tweets.find((ref) =>
        ref.type === "replied_to"
      );
      if (repliedTweet) {
        ...
        return {
          ...
          replyText:
            `I couldn't find the tweet you're trying to save. Please make sure you're replying to a valid tweet.`,
        };
      }
      ...
      console.log(
        `🔍 Checking if tweet ${originalTweetId} has already been saved...`,
      );
      const alreadySaved = await this._storageService.isTweetSaved(
        originalTweetId,
      );

      if (alreadySaved) {
        const savedTweetInfo = await this._storageService.getSavedTweet(
          originalTweetId,
        );
        console.log(
          `⚠️ Tweet ${originalTweetId} was already saved by @${savedTweetInfo?.reviewerUsername} on ${savedTweetInfo?.savedAt}`,
        );
        ...
        return {
          ...
          replyText:
            `This tweet has already been saved onchain by ${originalSaverName}`,
        };
      }

      console.log(
        `✅ Tweet ${originalTweetId} has not been saved before, proceeding...`,
      );

      // Fetch the original tweet details
      console.log(
        `📄 Fetching original tweet details for ID: ${originalTweetId}`,
      );
      const originalTweet = await this.twitterService.getTweetById(
        originalTweetId,
      );
      ...
      if (!originalTweet) {
        return {
          ...
          replyText:
            `I couldn't fetch the details of the original tweet. Please make sure you're replying to a valid tweet.`,
        };
      }

      // Get original tweet author information
      const originalAuthor = allUsers?.find((user) =>
        user.id === originalTweet.author_id
      );
      ...
      const isSelfReview =
        targetUsername.toLowerCase() === mentionerUsername.toLowerCase();
      const isConvertedPositiveSelfReview = isSelfReview &&
        reviewScore === "negative" && command.args.includes("positive");
      const originalTweetLink = `https://x.com/${
        originalAuthor?.username || "user"
      }/status/${originalTweetId}`;

      if (isConvertedPositiveSelfReview) {
        // Use anti-abuse message for converted positive self-reviews
        reviewTitle =
          "User tried to abuse Ethos and review themself positively through my code";
        reviewDescription = "Please don't do that again, anon.";
        console.log(
          `🚨 Using anti-abuse title and description for converted positive self-review by @${mentionerUsername}`,
        );
      } else {
        ...
        reviewDescription =
          `Original tweet saved by @${mentionerUsername}: "${originalTweet.text}"
...
Link to tweet: ${originalTweetLink}`;
      ...
      console.log(
        `📝 Review details - Score: ${reviewScore}, Title: ${reviewTitle}`,
      );
      console.log(
        `📝 Review description length: ${reviewDescription.length} characters`,
      );
      ...
      if (reviewResult.success) {
        ...
        await this._storageService.markTweetSaved(
          originalTweetId,
          targetUsername,
          mentionerUsername,
          reviewScore,
        ...
        await this._storageService.recordCommandUsage(
          mentionerUserId,
          mentionerUsername,
          "save",
        );
        ...
        if (reviewResult.data) {
          if (
            reviewResult.data.reviewUrl || reviewResult.data.url ||
            reviewResult.data.link
          ) {
            // Direct URL provided
            reviewLink = ` ${
              reviewResult.data.reviewUrl || reviewResult.data.url ||
            ...
            reviewResult.data.reviewId || reviewResult.data.id ||
            reviewResult.data.reviewID
          ) {
            // Construct the review link using the review ID
            const reviewId = reviewResult.data.reviewId ||
              reviewResult.data.id || reviewResult.data.reviewID;
            reviewLink = ` https://app.ethos.network/review/${reviewId}`;
          } else if (
            reviewResult.data.attestationUID || reviewResult.data.uid
          ) {
            // Some APIs use attestation UID for review linking
            const uid = reviewResult.data.attestationUID ||
              reviewResult.data.uid;
        ...
        if (!reviewLink) {
          const txHash = (reviewResult.data &&
            (reviewResult.data.tx || reviewResult.data.transactionHash)) ||
            "";
          const isValidTx = /^0x[a-fA-F0-9]{64}$/.test(txHash);
          if (isValidTx) {
            console.log(
              "⏳ No review link in response. Waiting 10s then checking Ethos activities by tx hash...",
            );
            await new Promise((resolve) => setTimeout(resolve, 10_000));
            const activityResult = await this.ethosService.getActivityByTx(
              "review",
              txHash,
            );
            if (activityResult.success && activityResult.data) {
              ...
              if (reviewIdFromData) {
                linkCandidate =
                  `https://app.ethos.network/activity/review/${reviewIdFromData}`;
              } else {
                // Legacy or alternative shapes as fallback
                linkCandidate = activity.url || activity.reviewUrl ||
                  activity.link || "";
                if (!linkCandidate) {
                  const idCandidate = activity.reviewId || activity.id ||
                    activity.reviewID || activity.attestationUID ||
                    activity.uid;
                  if (idCandidate) {
                    linkCandidate =
                      `https://app.ethos.network/activity/review/${idCandidate}`;
                  }
                }
              }
              if (linkCandidate) {
                ...
                console.log(
                  "⚠️ Activity fetched by tx but no usable link or id present in payload",
                );
              }
            } else {
              console.log(
                `⚠️ Could not fetch activity by tx ${txHash}: ${
                  activityResult.error || "unknown error"
                }`,
              );
            }
          } else {
            console.log(
              "ℹ️ No valid tx hash present in Ethos response; skipping activity lookup",
            );
          }
        }
        ...
        const phraseIndex =
          parseInt(originalTweetId.slice(-2), 10) % successPhrases.length || 0;
        const baseMessage = successPhrases[phraseIndex];
        const finalMessage = reviewLink
          ? `${baseMessage} View it here${reviewLink}`
          : baseMessage;
  ...
  /**
   * Extract the question text from a tweet, stripping leading @mentions
   */
  private extractQuestionFromTweet(text: string): string {
  ...
  /**
   * Handle the 'ask' command — natural language questions about Ethos data
   */
  private async handleAskCommand(
    ...
    try {
      ...
      // Check rate limit
      const rateLimitReason = await this.askRateLimiter.checkLimit(
        mentionerUserId,
      );
      if (rateLimitReason) {
        console.log(
          `🚨 Ask rate limit hit for @${mentionerUsername}: ${rateLimitReason}`,
        );
        return {
          success: false,
          message: "Ask rate limit exceeded",
          replyText:
            "You've reached the limit for questions this hour. Try again later.",
        };
      }

      // Extract mentioned usernames (excluding @ethosAgent)
      const mentionedUsernames = (tweet.text.match(/@\w+/g) || [])
        .map((m: string) => m.substring(1).toLowerCase())
        .filter((u: string) => u !== "ethosagent");

      // Determine reply-to user for contextual questions ("is this person trustworthy?")
      let replyToUser: string | undefined;
      if (tweet.in_reply_to_user_id) {
        const originalAuthor = allUsers?.find((u) =>
          u.id === tweet.in_reply_to_user_id
        );
        if (originalAuthor) {
          replyToUser = originalAuthor.username;
        }
      }

      // Extract the full question text (strip @mentions)
      const questionText = command.questionText ||
        this.extractQuestionFromTweet(tweet.text);
      ...
      if (!questionText) {
        return {
          ...
          replyText:
            'I didn\'t catch a question there. Try something like "how many vouches does @user have?"',
        };
      }

      console.log(
        `❓ Processing ask command from @${mentionerUsername}: "${questionText}"`,
      );

      // Call the ask service
      const result = await this.askService.handleQuestion(
        questionText,
        mentionedUsernames,
        mentionerUsername,
        replyToUser,
      ...
      // Non-Ethos questions — no rate limit recording, no Slack notification
      if (!result.isEthosQuestion) {
        return {
          success: true,
          message: "Non-Ethos question",
          replyText: result.replyText,
        };
      }

      // Record usage on success (even partial success — we used LLM + CLI resources)
      await this.askRateLimiter.recordUsage(mentionerUserId);

      // Send Slack notification for errors that warrant attention
      if (
        result.error &&
        !["planning_failed", "llm_not_configured"].includes(result.error)
      ) {
        await this.slackService.notifyError(
          "ask command",
          result.error,
          `@${mentionerUsername} asked: "${questionText}"`,
        );
      }
      ...
      return {
        ...
        message: result.error
          ? `Ask error: ${result.error}`
          : "Ask command processed",
        replyText: result.replyText,
      ...
      await this.slackService.notifyError(
        "ask command",
        error instanceof Error ? error.message : String(error),
        `@${command.mentionedUser.username} using ask command`,
      );

      return {
        success: false,
        message: "Error processing ask command",
        replyText: this.getStandardErrorMessage(),
      };
    }
  }
